...
int main(int argc, char* argv[]) {
  ...
  int* buffer2 = malloc(sizeof(int) * BUFFER_SIZE * 1024 + 100);
  if (!buffer2) {
    rc = EXIT_FAILURE;
    goto free_buffer;
  }
  ...
  if (comm_rank % 2 == 0) {
    if (comm_rank < comm_size - 1) {
      ...
      for (int i = 0; i <= 10; ++i) {
        ...
        for (int j = 0; j < 100; ++j) {
          MPI_Sendrecv(buffer, size, MPI_INT, comm_rank + 1, 10, buffer2,
                       size + 100, MPI_INT, comm_rank + 1, 20, MPI_COMM_WORLD,
                       &status);
    ...
    for (int i = 0; i <= 10; ++i) {
      for (int j = 0; j < 100; ++j) {
        MPI_Sendrecv(buffer, size, MPI_INT, comm_rank - 1, 20, buffer2,
                     size + 100, MPI_INT, comm_rank - 1, 10, MPI_COMM_WORLD,
                     &status);
        // MPI_Recv(buffer, size + 100, MPI_INT, comm_rank - 1, 10,
        // MPI_COMM_WORLD, &status); MPI_Send(buffer, size, MPI_INT, comm_rank -
        // 1, 20, MPI_COMM_WORLD);
      }
      size *= 2;
